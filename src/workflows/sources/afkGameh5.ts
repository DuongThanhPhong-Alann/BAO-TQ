...
async function fetchDetail(params: {
  ...
  retries: number;
  waitBetweenTriesMs: number;
}): Promise<Pick<AfkGameh5Item, "language" | "graphics" | "vote" | "installation_file">> {
  ...
  for (let page = pageFrom; page <= pageTo; page++) {
    ...
    try {
      ...
      for (const item of items) {
        ...
        seen.add(item.link);
        out.push(item);
        logger.info({ index: out.length, url: item.link }, "AFK gameh5: collected item");
  ...
  async function runWorker(workerId: number): Promise<void> {
    while (true) {
      ...
      try {
        ...
        logger.warn({ err, url: item.link, worker: workerId }, "AFK gameh5: failed to fetch detail");
      }
